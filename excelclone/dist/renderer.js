export class Renderer {
     ...
     * @param container
     */
    constructor(totalRows, totalColumns, cellWidth, cellHeight, canvas, container) {
    ...
    drawGrid() {
        ...
        const endCol = Math.min(this.totalColumns - 1, Math.ceil((scrollLeft + this.canvas.width) / this.cellWidth));
        const startRow = Math.floor(scrollTop / this.cellHeight);
        const endRow = Math.min(this.totalRows - 1, Math.ceil((scrollTop + this.canvas.height) / this.cellHeight));
